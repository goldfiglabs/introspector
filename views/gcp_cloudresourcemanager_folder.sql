DROP MATERIALIZED VIEW IF EXISTS gcp_cloudresourcemanager_folder CASCADE;

CREATE MATERIALIZED VIEW gcp_cloudresourcemanager_folder AS
...
FROM ( SELECT DISTINCT
    ...
    LEFT JOIN cte_resourceattrs AS _clsc_2 ON _clsc_2.uri = _key.uri
        ...
        AND _clsc_2.attr_name = 'displayname'
    ...
    LEFT JOIN cte_resourceattrs AS _clsc_5 ON _clsc_5.uri = _key.uri
        ...
        AND _clsc_5.attr_name = 'parent' WITH NO DATA;

REFRESH MATERIALIZED VIEW gcp_cloudresourcemanager_folder;

COMMENT ON MATERIALIZED VIEW gcp_cloudresourcemanager_folder IS 'GCP folders and their associated attributes.'
