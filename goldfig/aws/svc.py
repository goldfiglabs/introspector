...
from goldfig.aws import (ProxyBuilder, account_paths_for_import,
...
RegionalDbImportFn = Callable[[Session, int, str, ProxyBuilder], None]

Accounts = List[Tuple[str, ProviderCredential]]
RegionalPoolImportFn = Callable[
    # Any -> proxy_builder_args
    [f.ProcessPoolExecutor, Any, int, str, PathStack, Accounts],
    List[f.Future]]

GlobalResourceFn = Callable[[ServiceProxy], Iterator[Tuple[str, Any]]]

GlobalPoolImportFn = Callable[
    [f.ProcessPoolExecutor, Any, int, PathStack, Accounts], List[f.Future]]
...
def _global_async_proxy(ps: PathStack, proxy_builder_args, import_job_id: int,
                        config: Dict, svc_name: str,
                        import_fn: GlobalResourceFn):
  db = import_session()
  proxy_builder = make_proxy_builder(*proxy_builder_args)
  boto = load_boto_session_from_config(config)
  proxy = proxy_builder(boto)
