...
def _import_certificate_authorities(proxy: ServiceProxy) -> Iterator[Tuple[str, Any]]:
  certificate_authorities_resp = proxy.list('list_certificate_authorities')
  if certificate_authorities_resp is not None:
    certificate_authorities = certificate_authorities_resp[1]['CertificateAuthorities']
    for certificate_authority in certificate_authorities:
      yield 'CertificateAuthority', _import_certificate_authority(proxy, certificate_authority)
