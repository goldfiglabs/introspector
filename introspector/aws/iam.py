...
from introspector import ImportWriter
from introspector.mapper import MappedAttribute, MappedResource
from introspector.aws.fetch import Proxy, ServiceProxy
from introspector.aws.svc import GlobalService, GlobalResourceSpec, ServiceSpec, resource_gate
from introspector.delta.resource import apply_mapped_attrs
...
def _import_password_policy(proxy: ServiceProxy, spec: ServiceSpec):
  if resource_gate(spec, 'PasswordPolicy'):
...
def _import_instance_profiles(proxy: ServiceProxy, spec: ServiceSpec):
  if resource_gate(spec, 'instance-profile'):
